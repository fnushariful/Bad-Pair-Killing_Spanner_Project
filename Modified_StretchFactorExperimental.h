...
#ifndef SP_MER_MODIFIED_STRETCHFACTOREXPERIMENTAL_H
#define SP_MER_MODIFIED_STRETCHFACTOREXPERIMENTAL_H
...
namespace spanners {
    ...
    number_t Modified_StretchFactorExpDijk(PointIterator pointsBegin,
                                  PointIterator pointsEnd,
                                  EdgeIterator edgesBegin,
                                  EdgeIterator edgesEnd,
                                  vector<spanners::Edge> &added,
                                  map<int, vector<int>> &adjMap,
                                  const map<Point, int> &PointToIndices,
                                  const vector<Point> &perimeter,
                                  double t,
                                  const size_t numberOfThreads = 4) {
        vector<Point> P(pointsBegin, pointsEnd);
        const vector<Edge> E(edgesBegin, edgesEnd);
        const index_t n2 = perimeter.size();
...
//#pragma omp parallel for num_threads(numberOfThreads)
        ...
        for(auto p : perimeter){
            ...
            index_t u = PointToIndices.find(p)->second;
            ...
            do {
                ...
                while(!frontier.empty() && !open.empty()) {
                    ...
                    if( contains(frontier,v) ) {
                        ...
                        if(t_v > t){
                            added.template emplace_back(make_pair(u,v));
                            adjMap[u].template emplace_back(v);
                            adjMap[v].template emplace_back(u);
                            G[u].insert(v);
                            G[v].insert(u);
                            shortestPathLength[u] = getDistance(P[u], P[v]);
                            shortestPathLength[v] = getDistance(P[u], P[v]);
                            edgeAdded = true;
                            t_v = 1;
                        }
            ...
            } while(!bfs.empty() && (!(t_u > t_lvl) || edgeAdded)); ////
...
#endif //SP_MER_MODIFIED_STRETCHFACTOREXPERIMENTAL_H
