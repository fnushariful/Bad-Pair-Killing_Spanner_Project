...
namespace spanners {
    ...
    void point_partition(vector<Point> &P, CellToPointsMap &P_k, pair<number_t, number_t> &dimensions, pair<number_t, number_t> &adjustments,int size = 500) {
        ...
        for (auto& cell : P_k) {
            ...
            for (size_t cone = 0; cone < 8; cone++) {
                ...
                longIntPair neighbor = (occupiedTable[y_pos][x_pos] == 1) ? std::make_pair(x_pos, y_pos) : standard;
